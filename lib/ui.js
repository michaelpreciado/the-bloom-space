...
/**
 * The header reverses out white over the hero photograph and takes on a cream
 * surface once it is past it. Pages without a hero get the surface from the
 * first pixel.
 *
 * Driven by an observer on the hero rather than a scroll listener — the whole
 * motion system on this page depends on nothing running per frame on the main
...
export function initHeaderState() {
  ...
  new IntersectionObserver(([entry]) => {
    // Solid as soon as the header is over anything but the hero.
    header.classList.toggle('is-solid', entry.intersectionRatio < 0.02);
    // And park the hero's ambient motion while it is off screen — see the
    // .hero.is-idle rules. Nothing there is worth a frame you cannot see.
    hero.classList.toggle('is-idle', !entry.isIntersecting);
  }, { threshold: [0, 0.02, 0.2] }).observe(hero);
}

/**
 * The phone-only booking bar.
 *
 * Two rules, and the second matters as much as the first: show it once the
 * opening screen is behind you, and hide it again whenever a real "Request a
 * Viewing" button is on screen. The bar exists to carry the ask through the
 * middle of the page — stacked under the page's own button it is just a
 * second identical button, which reads as a mistake.
 *
 * Both run on IntersectionObserver rather than scroll listeners: the bar sits
 * over a page whose whole motion system is built on never touching the main
 * thread while scrolling.
 */
export function initBookBar() {
  ...
  const update = () =>
    bar.classList.toggle('is-visible', pastOpening && onScreen.size === 0);

  new IntersectionObserver(([entry]) => {
    pastOpening = entry.boundingClientRect.top < 0 && entry.intersectionRatio < 0.35;
    update();
  }, { threshold: [0, 0.35, 1] }).observe(anchor);

  // The page's own booking buttons, including ones the calendar renders later.
  const rivals = new IntersectionObserver((entries) => {
    entries.forEach((e) => {
  ...
  const watch = (root) => {
    root.querySelectorAll?.('a.btn-primary[href^="contact.html"]').forEach((a) => {
      if (!bar.contains(a)) rivals.observe(a);
    });
  };
  watch(document);

  // The availability panel re-renders its request link as dates are picked.
  const detail = document.getElementById('calDetail');
  if (detail) new MutationObserver(() => watch(detail)).observe(detail, { childList: true });
}
...
export async function initAvailability({ onSelect, onSlot } = {}) {
  ...
  function renderDetail(key) {
    ...
    if (!spans.length) {
      ...
      html += '</div>';
      html += `<div class="cal-request">
        <a class="btn btn-primary" href="contact.html?date=${key}">${t('pricing.requestThisDate')}</a>
      </div>`;
    }
    detail.innerHTML = html;
  }

  /* The hour chips looked tappable and did nothing. Choosing one now sets the
     estimate above and carries the start time into the request link, so the
     form arrives already knowing the day and the hour. */
  detail?.addEventListener('click', (e) => {
    const chip = e.target.closest('.cal-slot');
    if (!chip) return;
    detail.querySelectorAll('.cal-slot').forEach((c) =>
      c.classList.toggle('is-chosen', c === chip));

    const startMin = Number(chip.dataset.start);
    const link = detail.querySelector('.cal-request a');
    if (link && selected) link.href = `contact.html?date=${selected}&start=${startMin}`;
    onSlot?.(selected, startMin);
  });
